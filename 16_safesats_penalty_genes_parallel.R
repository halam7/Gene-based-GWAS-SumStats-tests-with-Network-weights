...
run_sats_analysis <- function(sumstats_path, setid, bfile, gene_list, chunk_id) {
  ...
  for (gene in names(setid_wide)) {
    tryCatch({
      ...
      result_uniform <- safe_sats(Z = Z, R = LD, W = NULL, timeout = 60, label = paste0("U-", gene))
      ...
      # Write immediately
      out_file <- file.path(here("data", "processed", "results_null"), paste0(trait, "_chunk_", chunk_id, ".txt"))
      fwrite(result_row,
             file = out_file,
             sep = "\t",
